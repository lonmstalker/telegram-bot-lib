 ...
 * <p>
 * Проверяют общий размер, минимальное разрешение и SafeSearch.
 */
...
public final class PhotoValidators {
    ...
    public static Validator<@NonNull List<PhotoSize>> minResolution(int w, int h) {
        return Validator.of(
                list -> list.stream()
                        .allMatch(ps -> ps.getWidth() >= w && ps.getHeight() >= h),
    ...
    public static Validator<@NonNull List<PhotoSize>> safeSearch() {
        return Validator.of(
                list -> MOD == null || list.stream()
                        .allMatch(ps -> MOD.isImageSafe(ps.getFileId())),
