 ...
 * <p>
 * Проверяет, что широта и долгота находятся в допустимых пределах.
 */
...
public final class LocationValidators {
    ...
    public static Validator<@NonNull Location> inBounds() {
        return Validator.of(
                loc -> loc.getLatitude() >= LAT_MIN && loc.getLatitude() <= LAT_MAX
                        && loc.getLongitude() >= LNG_MIN && loc.getLongitude() <= LNG_MAX,
