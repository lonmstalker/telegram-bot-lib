...
public final class AdvancedValidators {
    ...
    private static final List<DateTimeFormatter> FMTS = List.of(
    ...
    /**
     * Проверяет, что вся строка соответствует формату "<amount> <CURRENCY>"
     * где CURRENCY ∈ allowedCurrencies и amount > 0.
     *
    ...
    /**
     * Проверяет, что текст — это ISO-дата (YYYY-MM-DD) или DD.MM.YYYY,
     * и дата находится между сегодня (включительно) и завтра+maxDaysAhead.
     *
    ...
    /**
     * Проверяет, что определённый внешний сервисом язык текста
     * входит в множество разрешённых.
     *
    ...
    /**
     * Проверяет, что пользователь, инициировавший запрос (BotRequest.user.id),
     * совпадает с полем from в Telegram Message.
     *
     * @return Validator<BotRequest < ?>>
     */
    public static Validator<@NonNull BotRequest<?>> payerMatchesAuthor() {
        return Validator.of(req ->
                        Objects.equals(req.user().userId(), ((Message) req.data()).getFrom().getId()),
    ...
    /**
     * Проверяет, что timestamp Telegram Message (getDate) не отличается
     * от текущего времени более чем на maxSeconds секунд.
     *
