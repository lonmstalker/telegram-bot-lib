 ...
 * <p>
 * Содержат проверки на пустоту, максимальную длину,
 * корректное UTF-8 кодирование и опциональную Cloud-модерацию (toxic/profanity).
 */
...
public final class TextValidators {
    ...
    public static @NonNull Validator<String> regex(@NonNull Pattern pattern) {
        return Validator.of(s -> s != null && pattern.matcher(s).matches(),
                MessageKey.of("error.invalidRegex"));
    }
    ...
    public static @NonNull Validator<Integer> range(int min, int max) {
        return Validator.of(i -> i != null && i >= min && i <= max,
                MessageKey.of("error.invalidRange", min, max));
    }
    ...
    public static Validator<@NonNull String> notBlank() {
        return Validator.of(s -> !s.isBlank(),
                MessageKey.of("error.text.blank"));
    }
    ...
    public static Validator<@NonNull String> maxLength(int length) {
        return Validator.of(s -> s.length() <= length,
                MessageKey.of("error.text.tooLong", length));
    }
    ...
    public static Validator<@NonNull String> noToxicity() {
        return Validator.of(s -> MOD == null || !MOD.isToxicText(s),
                MessageKey.of("error.text.toxic"));
    }
    ...
    public static Validator<@NonNull String> noProfanity() {
        return Validator.of(s -> MOD == null || !MOD.isProfaneText(s),
                MessageKey.of("error.text.profanity"));
    }
}