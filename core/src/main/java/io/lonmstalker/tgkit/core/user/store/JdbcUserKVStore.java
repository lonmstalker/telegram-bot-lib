 ...
 * <p>Поддержка UPSERT’ов:</p>
 * <ul>
 *   <li>PostgreSQL 9.5+  (`INSERT … ON CONFLICT`)</li>
 *   <li>MySQL 5.7 / MariaDB 10.3 (`ON DUPLICATE KEY UPDATE`)</li>
 *   <li>Oracle 12c+      (`MERGE INTO … USING dual`)</li>
 *   <li>H2 / Derby       (`MERGE INTO … VALUES`)</li>
 *   <li>fallback (любая БД) — UPDATE + INSERT, если UPDATE = 0</li>
 * </ul>
 */
public final class JdbcUserKVStore implements UserKVStore {
    ...
    private enum Mode {PG, MYSQL, ORACLE, H2_DERBY, GENERIC}
    ...
    private static @NonNull String buildUpsertSql(Mode m) {
        return switch (m) {
            case PG -> "INSERT INTO user_kv(user_id,k,v) VALUES (?,?,?) " +
                    "ON CONFLICT (user_id,k) DO UPDATE SET v = EXCLUDED.v";
            case MYSQL -> "INSERT INTO user_kv(user_id,k,v) VALUES (?,?,?) " +
