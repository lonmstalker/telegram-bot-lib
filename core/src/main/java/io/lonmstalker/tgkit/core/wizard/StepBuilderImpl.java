...
import io.lonmstalker.tgkit.validator.impl.LocationValidators;
import io.lonmstalker.tgkit.validator.impl.PhotoValidators;
import io.lonmstalker.tgkit.validator.impl.TextValidators;
import io.lonmstalker.tgkit.validator.impl.VideoValidators;
 ...
 * &nbsp;• <em>type-validator</em> — убеждаемся, что в {@link BotRequest#data()} пришёл нужный тип;<br>
 * &nbsp;• <em>content-validator</em> — бизнес-ограничения (размер, разрешение и т.д.).</p>
 */
...
final class StepBuilderImpl<M, I, O> implements StepBuilder<M, I, O> {
    ...
    public @NonNull StepBuilder<M, String, String> expectText() {
        ...
        def.setTypeValidator(Validator.of(
    ...
    public @NonNull StepBuilder<M, String, Integer> expectInt() {
        def.setParser(req ->
                (I) Integer.valueOf(Integer.parseInt(((Message) req.data()).getText().trim())));
        def.setTypeValidator(Validator.of(
                r -> r.data() instanceof Message msg
                        && msg.hasText()
