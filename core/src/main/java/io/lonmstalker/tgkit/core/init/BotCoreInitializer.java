 ...
 * Вызывается ровно один раз на старте приложения — например,
 * в вашем {@code main()} или {@code @SpringBootApplication}.
 *
 * <p>👉 Не обязательно использовать этот класс — конфиг можно править вручную.
 * Но он наглядно демонстрирует, как корректно обновлять глобальное состояние.</p>
 */
@Slf4j
@UtilityClass
public class BotCoreInitializer {
    ...
    public synchronized void init() {
        if (started) {
            log.warn("[core-init] BotCoreInitializer уже вызывался, повторная инициализация игнорируется");
            return;
        }
