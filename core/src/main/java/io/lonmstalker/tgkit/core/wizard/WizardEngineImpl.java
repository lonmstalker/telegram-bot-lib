...
/**
 * Реализация {@link WizardEngine}, повторно использующая BotCommand-механизм
 * для навигационных shortcut’ов (&laquo;back&raquo;, &laquo;skip&raquo;, &laquo;cancel&raquo; и др.).
 *
 * <p>Алгоритм:</p>
 * <ol>
 *   <li>При регистрации Wizard создаём CommandAdapter-ы<br>
 *       для всех сгенерированных handler-методов навигации.</li>
 *   <li>Эти виртуальные методы прогоняются через все {@link BotCommandFactory}
 *       точно так же, как обычные команды приложения.</li>
 *   <li>В {@link #route(BotRequest)} сначала ищем BotCommand (shortcut),
 *       затем — выполняем текущий step по sessionId.</li>
 * </ol>
 */
@Slf4j
...
public final class WizardEngineImpl implements WizardEngine {
    ...
    public void route(@NonNull BotRequest<?> req) {
        ...
        SessionState st = stateStore.find(sid)
    ...
    public void resume(@NonNull BotRequest<?> req) {
        stateStore.findActiveByUser(req.chatId())
                .ifPresent(st -> {
                    Wizard<?> w = wizardMap.get(st.wizardId());
                    stepRunner.askStep(w.meta().id(), st.sessionId(),
                            w.steps().get(st.stepId()), req);
                });
    }
