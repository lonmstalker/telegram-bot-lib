...
class DslTtlIntegrationTest {
    ...
    void init() {
        ...
        when(sender.execute((PartialBotApiMethod<? extends Serializable>) any())).thenAnswer(inv -> {
            var sm = (SendMessage) inv.getArgument(0);
            Message m = new Message();
            Chat c = new Chat(); c.setId(Long.parseLong(sm.getChatId()));
            m.setChat(c);
            m.setMessageId(777);
            return m;
        });
    ...
    void ttlSchedulesDeletion() {
        ...
        Awaitility.await()
                ...
                .untilAsserted(() ->
                        verify(sender).execute(isA(DeleteMessage.class)));
