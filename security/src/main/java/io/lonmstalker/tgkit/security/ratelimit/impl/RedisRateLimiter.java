 ...
 *
 * <p>Использует атомарный Lua-скрипт (INCR + EXPIRE) — производительность
 * ≈ 1 мс, корректная работа в Redis Cluster и на Replica-set.</p>
 *
 * <h3>Lua-скрипт</h3>
 * <pre>
 ...
 * <h3>Плюсы</h3>
 * <ul>
 *   <li>Линейная масштабируемость — все экземпляры бота видят общий лимит.</li>
 *   <li>Никаких блокировок / WATCH / MULTI — одна команда <i>eval</i>.</li>
 * </ul>
 *
 * <h3>Минусы</h3>
 * <ul>
 *   <li>Фиксированное окно (fixed window) — возможен burst на границе.</li>
 *   <li>Нужен Redis server (stand-alone или кластер).</li>
 * </ul>
 *
...
public final class RedisRateLimiter implements RateLimiter {
    ...
    public boolean tryAcquire(@NonNull String key, int permits, int seconds) {
        ...
        try (Jedis jedis = pool.getResource()) {
            Object r = jedis.eval(LUA_SCRIPT, 1,
                    bucket,
