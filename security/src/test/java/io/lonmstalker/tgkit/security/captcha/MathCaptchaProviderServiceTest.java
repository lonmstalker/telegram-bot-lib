...
class MathCaptchaProviderServiceTest {
    ...
    @BeforeEach
    void setUp() {
        captcha = MathCaptchaProvider.builder()
        ...
        when(req.msgKey(anyString(), any(), any()))
                .thenCallRealMethod(); // позволит захватить key
    }
    ...
    void verifyTrueDeletesMsgAndClearsState() {
        ...
        when(req.delete(anyLong()).send()).thenAnswer(i -> {
            deleted.set(true);
            return null;
        });
