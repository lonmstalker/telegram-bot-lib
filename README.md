...
## 📁 Структура проекта
```text
core/         – реализация ядра и инфраструктуры
tgkit-api/    – публичные классы и DSL Telegram Bot API (`io.github.tgkit.api.*`)
api/          – общие интерфейсы (плагины, флаги, observability)
boot/         – Spring Boot стартер
experiment/   – экспериментальные функции под feature‑флагами
```

Пользовательский код обычно зависит только от `tgkit-api` и `core`. Остальные
модули можно подключать при необходимости.
...
Теперь бот доступен по адресу `http://0.0.0.0:8080/<TOKEN>`.
